    ...
    <div className="admin-page">
      ...
      <header className="admin-page-header admin-detail-heading">
        <div>
          ...
          <p>
            {dealer.contact_name} · signed up {formatDateTime(dealer.created_at)}
          </p>
        </div>
      ...
      <div className="admin-detail-grid">
        <section className="admin-detail-card admin-status-card">
          ...
          <select
            ...
            disabled={saving}
            onChange={(event) =>
              save({ status: event.target.value as Dealer["status"] }, `Status set to ${event.target.value}.`)
            }
          >
        ...
        {dealer.status === "pending" && (
          <section className="admin-detail-card admin-detail-wide">
            ...
            <p className="admin-muted">
              Approving only switches the account on. The dealer still has to complete onboarding — licence details,
              prefill data and their sale conditions — before they can run a sale.
            </p>
            <div className="button-row">
              <button
                ...
                disabled={saving}
                onClick={() => save({ status: "active" }, "Dealer approved.")}
              >
                Approve
              </button>
              <button
                ...
                disabled={saving}
                onClick={() => save({ status: "denied" }, "Dealer denied.")}
              >
                Deny
              </button>
            </div>
        ...
        <section className="admin-detail-card">
          ...
          <dl className="admin-detail-list">
            ...
            <div>
              <dt>Phone</dt>
              <dd>{dealer.phone ? <a href={`tel:${dealer.phone}`}>{dealer.phone}</a> : "Not supplied"}</dd>
            </div>
            <div>
              <dt>State</dt>
              <dd>{dealer.state_label}</dd>
            </div>
          </dl>
        </section>
        ...
        <section className="admin-detail-card">
          ...
          <dl className="admin-detail-list">
            <div>
              <dt>Plan</dt>
              <dd>{dealer.plan_label}</dd>
            </div>
            <div>
              <dt>Payment</dt>
              <dd>{dealer.payment_status_label}</dd>
            </div>
            <div>
              <dt>Current period ends</dt>
              <dd>{formatDateTime(dealer.subscription_current_period_end)}</dd>
            </div>
            <div>
              <dt>Cancels at period end</dt>
              <dd>{dealer.cancel_at_period_end ? "Yes" : "No"}</dd>
            </div>
            <div>
              <dt>Status</dt>
              <dd>{dealer.status_label}</dd>
            </div>
            <div>
              <dt>Status changed</dt>
              <dd>{formatDateTime(dealer.status_changed_at)}</dd>
            </div>
            <div>
              <dt>Signed up</dt>
            ...
            <div>
              <dt>Last updated</dt>
              <dd>{formatDateTime(dealer.updated_at)}</dd>
            </div>
          </dl>
        </section>
        ...
        <section className="admin-detail-card admin-detail-wide">
          ...
          <button
            type="button"
            className="admin-secondary-button"
            disabled={saving || notes === dealer.staff_notes}
